...
def entry_to_features(entry: ExoplanetEntry, dataset_type: DatasetType) -> dict:
    ...
    Convierte un ExoplanetEntry a un diccionario de características para el modelo.
    """
    def safe_value(quantity):
        #Safely extract the value from a Quantity object
        # Extrae de forma segura el valor de un objeto Quantity
        try:
            if quantity is None:
                return None
    ...
    # Características derivadas adicionales
...
def load_and_process_csv(csv_path: str) -> Tuple[List[dict], List[int], DatasetType]:
    ...
    Carga un CSV y procesa las entradas, devolviendo características y etiquetas.
    """
    ...
    for i, entry in enumerate(exoplanet_data.entries):
        try:            
            # Check if we have a valid disposition
            # Verificar si tenemos una disposición válida
            if entry.disposition is None:
                skipped += 1
                continue
            
            # Only use entries with clear disposition for training
            # Solo usar entradas con disposición clara para entrenamiento
            if entry.disposition in [Disposition.CANDIDATE, Disposition.CONFIRMED]:
                label = 1  # Positive (real planet) / Positivo (planeta real)
            elif entry.disposition == Disposition.FALSE_POSITIVE:
                label = 0  # Negative (false positive) / Negativo (falso positivo)
            else:
                # Skip ambiguous cases for training
                # Omitir casos ambiguos para entrenamiento
                skipped += 1
                continue
            ...
            skipped += 1
            if i < 5:  # Show only first few errors / Mostrar solo los primeros errores
...
def prepare_data_for_training(all_features: List[dict], all_labels: List[int]) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    Convierte listas de características y etiquetas al formato apropiado para entrenamiento.
    """
    ...
    # Manejar columna categórica 'dataset'
    if 'dataset' in df.columns:
        # One-hot encoding for dataset
        # Codificación one-hot para dataset
    ...
    # Rellenar valores NaN con -999 (LightGBM puede manejar esto como faltante)
    df = df.fillna(-999)
...
def train_model(X: pd.DataFrame, y: np.ndarray, validate: bool = True) -> Tuple[lgb.LGBMClassifier, dict]:
    ...
    Entrena un modelo LightGBM con validación cruzada opcional.
    """
    print("\n[INFO] Starting model training...")
    
    # Optimized parameters for exoplanet detection
    # Parámetros optimizados para detección de exoplanetas
    params = {
        'objective': 'binary',
        'metric': 'auc',
        'boosting_type': 'gbdt',
        ...
        'n_estimators': 500,
        'random_state': 42,
        'class_weight': 'balanced'  # Important for unbalanced datasets / Importante para datasets desbalanceados
    }
...
def calculate_optimal_thresholds(model: lgb.LGBMClassifier, X: pd.DataFrame, y: np.ndarray) -> dict:
    ...
    Calcula umbrales óptimos para clasificación basados en la precisión deseada.
    """
    ...
    # Encontrar umbral para alta precisión (pocos falsos positivos)
    ...
    # Encontrar umbral para alto recall (pocos falsos negativos)
...
def main():
    ...
    # Cargar y procesar todos los datasets disponibles
    all_features = []
    all_labels = []
